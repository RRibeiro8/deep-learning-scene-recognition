...
def main_worker(gpu, ngpus_per_node):
    ...
    if pretrained:
        ...
        if arch.startswith('efficientnet'):
            model = EfficientNet.from_pretrained(arch, num_classes=365)
        ...
        if net_exists:
            ...
            if arch.startswith('efficientnet'):
            	model = EfficientNet.from_name(arch, num_classes=365)
            else:
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr,  #model.fc.parameters()
                                momentum=momentum,
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            best_acc1 = checkpoint['best_acc1']#['best_prec1']
            if gpu is not None:
